...
class xiaomiFanAccessory {
  ...
  preapreFanServices() {
    ...
    this.informationService
      ...
      .setCharacteristic(Characteristic.FirmwareRevision, '0.9.5');
  ...
  checkFanState(callback) {
    ping.sys.probe(this.ip, (isAlive) => {
      if (!isAlive && this.fanDevice) {
        ...
        this.fanDevice = undefined;
        this.resetFanStatus(); // when lost connection to fan then it was probably manually turned off, in that case we need to reset the fan info.
        callback(false);
