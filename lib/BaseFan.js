...
class BaseFan extends EventEmitter {
  ...
  supportsPowerControl() {
    return this.capabilities[FanCapabilities.POWER_CONTROL] || true; // every fan supports that?
  }
  ...
  supportsFanSpeedRpm() {
    return this.capabilities[FanCapabilities.FAN_SPEED_RPM] || false; // whether the fan reports the rpm speed value
  }
  ...
  supportsOscillation() {
    return this.capabilities[FanCapabilities.OSCILLATION_CONTROL] || false; // fan moves left to right
  }
  ...
  supportsNaturalMode() {
    return this.capabilities[FanCapabilities.NATURAL_MODE] || false; // whether the natural mode is supported
  }

  supportsSleepMode() {
    return this.capabilities[FanCapabilities.SLEEP_MODE] || false; // whether the sleep mode is supported
  }

  supportsChildLock() {
    return this.capabilities[FanCapabilities.CHILD_LOCK] || false; // should be clear
  }
  ...
  supportsBuzzerControl() {
    return this.capabilities[FanCapabilities.BUZZER_CONTROL] || false; // if buzzer can be configured
  }

  supportsBuzzerLevels() {
    return this.capabilities[FanCapabilities.BUZZER_CONTROL_LEVELS] || false; // if buzzer can be set to different value
  }

  supportsLedControl() {
    return this.capabilities[FanCapabilities.LED_CONTROL] || false; // if indicator light can be configured
  }

  supportsLedLevels() {
    return this.capabilities[FanCapabilities.LED_CONTROL_LEVELS] || false; // if indicator light can be set to different value
  }
  ...
  supportsIoniser() {
    return this.capabilities[FanCapabilities.IONISER_CONTROL] || false; // whether the fan has a built in ioniser which can be controled
  }
