...
class MiotDmakerDcFan extends MiotFan {
  ...
  // dmaker.fan.p9, dmaker.fan.p10, dmaker.fan.p11, dmaker.fan.p15, dmaker.fan.p18
  ...
  // https://miot-spec.org/miot-spec-v2/instance?type=urn:miot-spec-v2:device:fan:0000A005:dmaker-p30:1
  // https://miot-spec.org/miot-spec-v2/instance?type=urn:miot-spec-v2:device:fan:0000A005:dmaker-p33:1
  // https://miot-spec.org/miot-spec-v2/instance?type=urn:miot-spec-v2:device:fan:0000A005:dmaker-p220:1
  ...
  initFanCapabilities() {
    ...
    if (this.model === 'dmaker.fan.p9') {
      ...
      // has slighlty different oscillation angle levels
  ...
  addFanProperties() {
    ...
    if (fanModel === 'dmaker.fan.p9') {
      ...
      // dmaker.fan.p33
      this.defineProperty('child_lock', 7, 1);
      this.defineProperty('fan_speed', 2, 6); // it is marked read only in the spec but somehow write also works...
      this.defineProperty('swing_mode', 2, 4);
      this.defineProperty('swing_mode_angle', 2, 5);
      this.defineProperty('power_off_time', 3, 1);
      this.defineProperty('buzzer', 5, 1);
  ...
  async setAngle(angle) {
    if (angle > 140) angle = 140; // the fans only support some predifened angles so i am not sure how this will beahve
    if (angle < 0) angle = 0;
