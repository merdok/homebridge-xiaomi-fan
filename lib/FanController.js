...
const EVENT_EMITTER_FAN_CONNECTED = 'fanConnected';
const EVENT_EMITTER_FAN_DISCONNECTED = 'fanDisconnected';
const EVENT_EMITTER_FAN_DEVICE_READY = 'fanDeviceReady';
const EVENT_EMITTER_FAN_PROPERTIES_UPDATED = 'fanPropertiesUpdated';
...
class FanController extends EventEmitter {
  ...
  startFanDiscovery() {
    ...
    miio.device({
      ...
      this.emit(EVENT_EMITTER_FAN_CONNECTED, this.fanDevice);
    }).catch(err => {
      this.logDebug(err);
  ...
  createFanDevice(miioDevice, model) {
    ...
    if ((miioDevice || model) && !this.fanDevice) {
      ...
      this.emit(EVENT_EMITTER_FAN_DEVICE_READY, this.fanDevice); // notify listeners that a fan device was created
  ...
  startFanPolling() {
    this.checkFanStatusInterval = setInterval(() => {
      this.fanDevice.pollProperties().then(result => {
        ...
        this.emit(EVENT_EMITTER_FAN_PROPERTIES_UPDATED, result);
      }).catch(err => {
        if (this.checkFanStatusInterval) {
          ...
          this.checkFanStatusInterval = undefined;
          this.fanDevice.disconnectAndDestroyMiioDevice();
          this.emit(EVENT_EMITTER_FAN_DISCONNECTED, null);
